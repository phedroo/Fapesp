...
#Adicionado para corrigir erro Error in `$<-`:
# ! Assigned data `value` must be compatible with existing data.
# ✖ Existing data has 10467 rows.
# ✖ Assigned data has 10466 rows.
# ℹ Only vectors of size 1 are recycled.
# Caused by error in `vectbl_recycle_rhs_rows()`:
#   ! Can't recycle input of size 10466 to size 10467.
...
# FAXINA PARA BANCO DE DADOS 021/024 (Climate TRACE atualizado)
# para posterior incorporação na primeira base ct
# arquivo emissions-sources -----------------------------------------------
# buscando o caminho dos setores
tbl_directorys <- as_tibble(
  list.files("data-raw/ct024/../", full.names = TRUE, recursive = TRUE)) %>%
  filter(str_detect(value, "emissions-sources.csv"))
